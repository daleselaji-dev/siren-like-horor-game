...
export function buildHotel(ctx) {
  ...
  wallZ(M.plaster, -11.1, 11.1, 17.1, ROOF, ROOF + 0.9);
  // 楼顶大招牌（钢架 + 灯箱）——轮19：架上中央退台体（4F 机房层）的顶上，
  // 招牌成为「中间体」的冠：广角剪影 = 低两翼 → 高中央体 → 更高的字架
  const ATTIC_T = ROOF + 2.55;   // 中央退台体顶（体量本体在轮19 块里砌）
  box(M.signSouth, 0, ATTIC_T + 0.98, 9.05, 10.5, 1.7, 0.25);
  for (const sx of [-4.5, 0, 4.5]) box(M.ironDark, sx, ATTIC_T + 0.55, 8.72, 0.12, 1.1, 0.12, 0, 0.3, 0);
  // 侧壁竖招牌
  ...
  // ================= 立面轮19：体量分级——中央退台体（4F 机房层）=================
  // 门3轮18终审「hotel_wide 仍像一条灰砖」的剪影级根治：楼不再是单一长方体——
  // 屋面正中砌一座 16×18.5m、高 2.55m 的退台体量（电梯机房/水箱间层，北面退进 1.7m），
  // 顶上架店名字架。广角剪影自此三级：低两翼(带天线/水箱/机房) → 中央体 → 字架。
  {
    const atticM = new THREE.MeshStandardMaterial({ color: 0x8f8a80, roughness: 0.9 }); // 退台体抹灰（断言标记色）
    box(atticM, 0, ROOF + 1.275, 0.25, 16.0, 2.55, 18.5);              // 体量本体（抹灰面）
    box(M.concreteDark, 0, ROOF + 2.605, 0.25, 16.36, 0.14, 18.86);    // 压顶挑檐（出挑一皮）
    box(M.concreteDark, 0, ROOF + 0.09, 0.25, 16.2, 0.18, 18.7);       // 底座圈梁
    // 北面三樘机房层小窗（绿钢框+暗玻带贴面）——退台体不是实心碑，是有层的「房」
    // （体量是实心盒：玻带/框系全部贴在面外 1-9cm，读法与主楼窗一致）
    {
      const frameA = new THREE.MeshStandardMaterial({ color: 0x37544a, roughness: 0.5, metalness: 0.25 });
      box(M.winRoom, 0, ROOF + 1.55, 9.53, 13.4, 0.9, 0.04);           // 暗腔玻璃带（贴面外皮）
      for (const [a, b] of [[-5.6, -3.8], [-0.9, 0.9], [3.8, 5.6]]) {
        const cx2 = (a + b) / 2, w2 = b - a;
        box(frameA, cx2, ROOF + 1.1, 9.56, w2, 0.05, 0.06);            // 下框
        box(frameA, cx2, ROOF + 2.0, 9.56, w2, 0.05, 0.06);            // 上框
        box(frameA, a + 0.025, ROOF + 1.55, 9.56, 0.05, 0.95, 0.06);   // 左框
        box(frameA, b - 0.025, ROOF + 1.55, 9.56, 0.05, 0.95, 0.06);   // 右框
        box(frameA, cx2, ROOF + 1.55, 9.56, 0.045, 0.95, 0.055);       // 竖中梃
        box(atticM, cx2, ROOF + 0.98, 9.6, w2 + 0.3, 0.12, 0.16);      // 窗台出挑
      }
      // 北墙雨渍两道 + 竖向伸缩缝——退台体与主楼同一场雨里老去
      box(M.concreteDark, -6.7, ROOF + 1.9, 9.52, 0.5, 1.2, 0.02);
      box(M.concreteDark, 5.9, ROOF + 2.2, 9.52, 0.35, 0.66, 0.02);
      box(M.ironDark, -2.4, ROOF + 1.275, 9.52, 0.06, 2.5, 0.02);
    }
    // 排风百叶两樘（东/西侧面）——机房层的功能证据
    for (const s of [-1, 1]) {
      box(M.ironDark, s * 8.03, ROOF + 1.5, 2.0, 0.06, 0.9, 1.6);
    }
  }
