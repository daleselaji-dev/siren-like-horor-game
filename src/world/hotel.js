...
export function buildHotel(ctx) {
  ...
  // 三层大盒有了横向楼层节奏与逐窗投影，远看不再是一整块贴了窗纸的板
  {
    ...
    // 空调外机下的锈滴痕
    for (const [wx, wy] of [[-10.6, F2 + 0.7], [2.6, F3 + 0.7], [7.1, F2 + 0.7], [-6.1, F3 + 0.7]]) {
      box(M.concreteDark, wx + 0.18, wy - 0.75, 11.06, 0.12, 0.95, 0.02);
    }
  }
  ...
  // ================= 立面轮18：真实进深模块——阳台挑板/空调机架/勒脚加高/水平分缝 =================
  // 门3审计「广角仍读成单盒」的体量级根治：30m 外雾里能读的只有「从墙里长出来的体量」——
  // 每层三樘窗改出挑阳台（挑板+实心栏板+扶手+侧颊+牛腿），六台空调外机架上钢三角架，
  // 勒脚加高成两皮基座带，层中加水平分缝——广角第一读是「格子里长满东西的旧楼」
  {
    const winsN = [[-15.5, -13.5], [-11.5, -9.5], [-7, -5], [-2.5, -0.5], [1.5, 3.5], [6, 8], [10.5, 12.5], [14, 16]];
    const balcM = new THREE.MeshStandardMaterial({ color: 0x837d6f, roughness: 0.88 }); // 阳台抹灰（断言标记色）
    const balconyAt = (cx2, y0) => {
      box(balcM, cx2, y0 + 0.90, 11.58, 2.7, 0.13, 0.92);              // 挑板（出墙 0.9m）
      box(M.concreteDark, cx2, y0 + 0.835, 11.58, 2.74, 0.02, 0.96);   // 板底滴水线（挑板下的暗缝）
      for (const s of [-1, 1]) {
        box(M.concreteDark, cx2 + s * 1.05, y0 + 0.74, 11.32, 0.16, 0.22, 0.36); // 牛腿托座
        box(balcM, cx2 + s * 1.315, y0 + 1.42, 11.76, 0.07, 0.92, 0.56);         // 侧颊板
      }
      box(balcM, cx2, y0 + 1.42, 12.0, 2.7, 0.92, 0.07);               // 前栏板（实心）
      box(M.concreteDark, cx2, y0 + 1.905, 12.0, 2.78, 0.06, 0.13);    // 扶手压顶
      box(M.concreteDark, cx2 + 0.5, y0 + 1.06, 12.045, 0.5, 0.5, 0.012); // 栏板外挂雨渍片
    };
    // 选位避开正门雨棚（|x|<7.7）与空调机位；0/7 两列上下层对齐——
    // 「成列的阳台」是砖混住宅楼的第一进深读法
    for (const bi of [0, 6, 7]) balconyAt((winsN[bi][0] + winsN[bi][1]) / 2, F2);
    for (const bi of [0, 3, 7]) balconyAt((winsN[bi][0] + winsN[bi][1]) / 2, F3);
    // 晾衣杆一根 + 搭着的旧布（有人住过的证据——2001 的阳台不是样板间）
    box(M.ironDark, (winsN[6][0] + winsN[6][1]) / 2, F2 + 1.62, 11.8, 2.3, 0.03, 0.03);
    box(M.clothRed, (winsN[6][0] + winsN[6][1]) / 2 - 0.5, F2 + 1.44, 11.8, 0.5, 0.36, 0.04);
    // —— 空调外机六台：钢三角架托底 + 格栅前脸 + 锈痕（原四台补架，再添两台）——
    const acGrill = new THREE.MeshStandardMaterial({ color: 0x565a5c, roughness: 0.6, metalness: 0.35 });
    const acUnits = [[-10.6, F2 + 0.7], [2.6, F3 + 0.7], [7.1, F2 + 0.7], [-6.1, F3 + 0.7], [-8.5, F3 + 0.7], [9.5, F2 + 0.7]];
    for (const [wx, wy] of acUnits) {
      ...
      box(M.concreteDark, wx + 0.18, wy - 0.75, 11.16, 0.12, 0.95, 0.014);   // 锈滴痕
    }
