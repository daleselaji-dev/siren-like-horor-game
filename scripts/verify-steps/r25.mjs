// 轮25 取证：门2（keeper/matron 灾难帧重做——刷状竖发/蓝珠眼/油亮暗肤/
//   孔板黑洞/颈衣缝隙全项根治）+ 门3（外墙污迹层/入口体积光/积水反射）
...
// 轮25·人像纪律：①颈仰角归零——idle 收敛后颈上仰 0.08-0.12 rad，0.6m 俯视角
//   下颌抬离领口读成「长颈+领内深洞」；肖像位钉平视。②补光改暖弱
//   （冷青补光在虹膜/角膜上洗出「蓝珠眼」）+ 机位背后一档暖底光抬暗部——
//   keeper 祠堂无环境灯，r24 里只吃单侧聚光渲成「油亮暗皮怪物」
import fs from 'node:fs';

export async function run(page, h) {
  fs.mkdirSync('verify/r25', { recursive: true });
  ...
  const closeup = async (id, name) => {
    await page.evaluate((cid) => {
      ...
      // 轮25：肖像位颈仰角/歪头归零——idle 的 0.08-0.12 rad 上仰在 0.6m
      // 近景里把下颌抬离领口（「长颈」+「领内黑洞」的取证机位放大器）
      hum.neck.rotation.x = 0;
      hum.head.rotation.z = 0;
      ...
      if (SpotC) {
        ...
        // 轮25：补光冷青(0xcfe0d8)→暖米(0xdcd2c4)——冷补光映在角膜/巩膜的
        // 湿层上正是 r24「小蓝珠眼」的色源；强度 14→18 抬暗部
        const fillL = new SpotC(0xdcd2c4, 18, 10, 0.9, 0.7);
        ...
        // 底光（暖、极弱、无影）：keeper 祠堂/matron 3F 没有环境灯，单侧聚光
        // 拍出「一半脸黑洞」的怪物感——一档假反弹让暗部有皮色
        let PointC = null;
